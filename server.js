...
mongoose.connect("mongodb://localhost/TestProject3");
...
// Route to see projects we have added
...
app.post("/byEmail", function(req,res){
  ...
  User.findOne({first: req.body.first})
    .populate('Project')
...
// New Project creation via POST route
app.post("/submit", function(req, res) {
  // Use our Project model to make a new note from the req.body
  var newProject = new Project(req.body);
  // Save the new note to mongoose
  newProject.save(function(error, doc) {
    // Send any errors to the browser
    if (error) {
      res.send(error);
    }
    // Otherwise
      ...
      User.findOneAndUpdate({first: req.body.first}, { $push: { "Project": doc._id } }, { new: true }, function(err, newdoc) {
        // Send any errors to the browser
        if (err) {
          res.send(err);
        }
